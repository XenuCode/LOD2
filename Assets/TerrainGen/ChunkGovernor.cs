...
//a no i jeszcze jedno egghh
    public void FixedUpdate()
    {
        ...
        {
            Chunk chunkz;
           // Debug.Log("TO REMOVE :"+toRemove.Count);
            if (toRemove.Count > 0)
            {
                loadedChunks.TryGetValue(toRemove[0], out chunkz);
                Destroy(chunkz.gameObject); //dosyć EXPENSIVE eeee refrence to gameobject i samo usunięcie
                loadedChunks.Remove(toRemove[0]);
                toRemove.RemoveAt(0);
            }
        }
                ...
                for (int y = -REND + (int)cameraChunkPosition.y; y <= REND + (int)cameraChunkPosition.y; y++)
                {
                    lock (loadedChunks)
                    {
                        lock (chunksToGenerate)
                        {
                            ...
                            {
                                if (loadedChunks.ContainsKey(new int2(x, y)) ||
                                    chunksToGenerate.ContainsKey(new int2(x, y)) ||
                                    chunksBeingGenerated.Contains(new int2(x, y)))
                                {
    ...
    IEnumerator ScanChunks()
    {
        //for unloading chunks as well as LOD 
        for(;;)
        {
            var pos = camera.transform.position / ((N - 1) * pointOffset);
                ...
                lock (toRemove)
                {
                    lock (loadedChunks)
                    {
                        foreach (var chunk in loadedChunks) //iterates over each element so yeee
                        {
                            ...
                            float distance = Vector2.Distance(cameraChunkPosition,
                                new Vector2(chunk.Value.chunkData.position.x, chunk.Value.chunkData.position.y));
    ...
    public class ChunkData
    {
        ...
        public ChunkData(int2 position, int chunkSize, Vector3[] vertecies, float pointOffset, int heightMultipl,
            int lodLvl)
        {
